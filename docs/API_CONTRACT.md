...
| `top_k` | integer | `8` | 最多采用 1～20 条候选证据 |
...
按检索顺序加入完整片段；预算不足时，最后一个可加入的片段会截短并标注“内容已截断”。标题等引用头信息也计入预算；首条片段不再拥有越过估算预算的例外。若连头信息和正文前缀都放不下，该候选会被跳过。

日志 `build_evidence` 记录 `candidates`、`used`、`estimated_tokens`、`budget`、`truncated`。检索到 8 条表示有 8 条候选，最终采用条数还取决于来源校验、去重和预算。
