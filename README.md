...
- **引用问答**：路由器判断是否需要生成回答；需要时复用检索结果，核对文本块存在且属于对应论文，再从 SQLite 读取正文与引用元数据、按预算组织证据，交给 LLM，最后检查 `[N]` 是否对应本轮来源。
...
`CITEQUEST_RAG_CONTEXT_TOKENS` 设置输入证据预算，默认 `8000` 个估算 token。检索卡片的 300 字符预览不会限制 RAG 读取的正文；证据超预算时会截短最后一个可加入的片段并标注。该预算不含系统提示、问题和输出，实际 token 数取决于所用模型。
